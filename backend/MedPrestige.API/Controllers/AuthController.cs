...
namespace MedPrestige.Api.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        {
            var existing = _bl.UserAction().GetByEmail(dto.Email);
            if (existing != null)
            ...
            var user = _bl.UserAction().GetByEmail(dto.Email);

            _bl.PatientAction().Add(new PatientDto { UserId = user.UserId });

            var patient = _bl.PatientAction().GetByUserId(user.UserId);
        ...
        {
            var user = _bl.UserAction().Login(dto.Email, dto.Password);
            if (user == null)
                return Unauthorized(new { message = "Invalid email or password." });

            var patient = _bl.PatientAction().GetByUserId(user.UserId);
            var doctor = _bl.DoctorAction().GetAll().FirstOrDefault(d => d.UserId == user.UserId);
